...
class TextAdapter(nn.Module):
    def __init__(
        ...
        ch5x5red: int,
        ch5x5: int,
        #hidden_size: int = 256,  # BiGRU 隐藏层大小
        output_dim: int = 64,    # 目标降维维度
        skip_connect=False,
    ) -> None:
        super().__init__()
        ...
        # === 添加 BiGRU 和 线性降维层 ===
        #self.bigru = nn.GRU(input_size=in_channels, hidden_size=hidden_size, 
                           # num_layers=1, batch_first=True, bidirectional=True)
        #self.fc_reduce = nn.Linear(2 * hidden_size, output_dim)  # 降维到 64
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outputs = torch.cat(outputs, dim=1).permute(0, 2, 1)  # [B, seq_len, D]
        #print(f"Before GRU: {outputs.shape}")  # 打印 GRU 处理前的维度

        # === BiGRU 层 ===
        #outputs, _ = self.bigru(outputs)  # [B, seq_len, 2*hidden_size]
        #print(f"After GRU: {outputs.shape}")  # 打印 GRU 处理后的维度

        # === 线性降维层 ===
        #outputs = self.fc_reduce(outputs)  # [B, seq_len, output_dim]
        #print(f"After Linear Reduction: {outputs.shape}")  # 打印降维后的维度

        clstoken = x0[:, 0:1, :]  # 取 CLS Token
