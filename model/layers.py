...
class CrossModalTransformer(nn.Module):
    def __init__(self, d_model=512, nhead=8, num_layers=4, dim_ffn=2048, dropout=0.1):
        super().__init__()
        self.layers = nn.ModuleList([
            nn.TransformerDecoderLayer(
                ...
                dim_feedforward=dim_ffn,
                dropout=dropout,
                batch_first=True  # 让 batch 维度在第一维
    ...
    def forward(self, query, key_value):
        '''
        query: b, 1, d_model   (全局文本特征 state)
        key_value: b, hw, d_model (视觉特征 fq)
        '''
        for layer in self.layers:
            query = layer(query, key_value)  # TransformerDecoderLayer 允许 query 和 memory 交互
