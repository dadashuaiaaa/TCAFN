...
def inference(test_loader, model, args):
    ...
    iou_list = []
    total_inter = 0  # 计算所有样本的总交集
    total_union = 0  # 计算所有样本的总并集
    ...
    for img, param in tbar:
        ...
        for sent in param['sents']:
            ...
            # IoU 计算
            inter = np.logical_and(pred, mask)  # 交集
            union = np.logical_or(pred, mask)   # 并集
            I = np.sum(inter)
            U = np.sum(union)
            iou = I / (U + 1e-6)
            iou_list.append(iou)

            # 累加 Overall IoU 需要的数值
            total_inter += I
            total_union += U
    ...
    # 输出日志
    logger.info('Mean IoU={:.2f}'.format(100. * miou))
    logger.info('Overall IoU={:.2f}'.format(100. * oiou))
    for k, v in prec.items():
        logger.info('{}: {:.2f}.'.format(k, 100. * v))

    return miou, oiou, prec
